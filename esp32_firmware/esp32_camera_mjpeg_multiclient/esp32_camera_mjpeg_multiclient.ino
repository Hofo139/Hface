...
const unsigned long snapshotInterval = 3000;  // každé 2 sekundy
const char* serverUrl = "http://173.249.44.137:5000/upload";
unsigned long lastPollMillis = 0;
const unsigned long pollInterval = 3000; // 3 sekundy
int selectedRecognitionMode = 1;
const char* wemosIps[] = {"192.168.0.92", "172.20.10.8"};
const int wemosIpCount = sizeof(wemosIps) / sizeof(wemosIps[0]);
void connectToWiFi(int networkIndex);
void showWifiMenu();
void showCameraMenu();
void showRecognitionMenu();
void showInfoMenu();
void showMenu();
...
void setup() {
  ...
  btStop(); 
  Serial.println("Rezim rozpoznavania (0 = server, 1 = app): " + String(selectedRecognitionMode));
...
void loop() {
  ...
  if (upPressed) {
    if (buttonUpPressedTime == 0) {
      buttonUpPressedTime = now;
    } else if (now - buttonUpPressedTime >= 1000) {
      navigateUp();
      buttonUpPressedTime = 0;
      while (digitalRead(BUTTON_UP) == LOW) delay(10);  // počkaj na uvoľnenie
    }
  } else {
    buttonUpPressedTime = 0;
  }
  ...
  if (downPressed) {
    if (buttonDownPressedTime == 0) {
      buttonDownPressedTime = now;
    } else if (now - buttonDownPressedTime >= 1000) {
      navigateDown();
  ...
  if (selectPressed) {
    if (buttonSelectPressedTime == 0) {
      buttonSelectPressedTime = now;
    } else if (now - buttonSelectPressedTime >= 1000) {
      selectOption();
  ...
  // --- Odošli snímku na server ---
  if (selectedRecognitionMode == 0) {
    if (now - lastSnapshotMillis >= snapshotInterval) {
      Serial.println("[LOOP] Odosielam snímku na server...");
...
void sendLedCommandToWemos(String color);

void sendSnapshotToServer() {
  cam.run();
  size_t imageSize = cam.getSize();
  uint8_t* imageData = cam.getfb();
  ...
  String response = http.getString();  // ← treba volať mimo if(), lebo inak niekedy vráti len "OK"
  response.trim();  // odstráni \n alebo medzery

  Serial.println("Server odpovedal: " + response);

  if (httpResponseCode == 200) {
    if (response == "NOFACE") {
      Serial.println("[INFO] Ziadna tvar detegovana – nic sa nerobi.");
      // Nerobime nic
    } else if (response.startsWith("RECOGNIZED:")) {
      String name = response.substring(String("RECOGNIZED:").length());
      sendRecognizeToOLED(name);

      if (selectedRecognitionMode == 0) {
        sendLedCommandToWemos("green");
      }

    } else if (response == "UNKNOWN") {
      sendRecognizeToOLED("NEZNAMY");

      if (selectedRecognitionMode == 0) {
        sendLedCommandToWemos("red");
      }

    } else {
      Serial.println("[WARN] Neznáma odpoveď zo servera: " + response);
    }
  } else {
    Serial.print("Chyba pri odosielaní fotky: ");
...
void sendRecognizeToOLED(String name) {
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(0, 0);
...
void sendRecognizeToServer(String name) {
  HTTPClient http;
  String url = "http://" + ipAddress + "/recognize?name=" + name;
  http.begin(url);
...
void showMenu() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        else if (i == 3) display.print("HFACE");
...
void navigateUp() {
  if (currentMenu == 0) {
    ...
    selectedInfoOption--;
    if (selectedInfoOption < 0) selectedInfoOption = 1;
    showRecognitionMenu();
  }
}
...
void navigateDown() {
  if (currentMenu == 0) {
    ...
    selectedInfoOption++;
    if (selectedInfoOption > 1) selectedInfoOption = 0;
    showRecognitionMenu();
  }
}
...
void showInfoMenu() {
    ...
    display.print("Rezim: ");
    display.print(selectedRecognitionMode == 0 ? "Server" : "App");
...
void showRecognitionMenu() {
  ...
  display.print("Rozpoznavanie");

  display.drawLine(0, 16, SCREEN_WIDTH, 16, WHITE);

  int y[] = {22, 36, 50};
  for (int i = 0; i < 2; i++) {
    if (selectedInfoOption == i) {
      display.fillRect(0, y[i] - 2, SCREEN_WIDTH, 12, WHITE);
      display.setTextColor(BLACK);
    } else {
      display.setTextColor(WHITE);
    }
    ...
    if (i == 0) display.print("SERVER");
    else if (i == 1) display.print("APP");
  }
...
void selectOption() {
  if (currentMenu == 0) {
    if (selectedOption == 0) {
      ...
      showInfoMenu();
    } else if (selectedOption == 3) {  // HFACE menu (Rozpoznávanie)
      currentMenu = 3;
      selectedInfoOption = 0;  // default výber: SERVER
      showRecognitionMenu();
    }
  } else if (currentMenu == 1) {  // Kamera menu
  ...
  } else if (currentMenu == 2) {  // WiFi menu
  ...
  } else if (currentMenu == 3) {  // Rozpoznávanie (HFACE)
    selectedRecognitionMode = selectedInfoOption;  // 0 = SERVER, 1 = APP
    currentMenu = 0;
    showMenu();
  }
}
...
void sendLedCommandToWemos(String color) {
  ...
  for (int i = 0; i < wemosIpCount; i++) {
    String url = "http://" + String(wemosIps[i]) + "/led?color=" + color;
    HTTPClient http;
    http.begin(url);
    ...
    if (httpResponseCode > 0 && httpResponseCode == 200) {
      Serial.println("[WEMOS] Odošlané na " + String(wemosIps[i]) + " → " + color);
      success = true;
      break;
    } else {
      Serial.println("[WEMOS] Chyba spojenia s " + String(wemosIps[i]) + " → Skúšam ďalšiu...");
    }
  }

  if (!success) {
    Serial.println("[WEMOS] Zlyhalo odoslanie požiadavky na všetky IP.");
  }
}
